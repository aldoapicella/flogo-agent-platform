...
describe("RunnerExecutorService", () => {
  ...
  it("executes helper-backed exact install diff planning and publishes a reviewable canonical diff artifact", async () => {
    process.env.FLOGO_HELPER_BIN = await createHelperScript(
      JSON.stringify({
        result: {
          contributionKind: "trigger",
    ...
    const result = await service.execute({
      ...
      envSecretRefs: {},
      timeoutSeconds: 60,
      artifactOutputUri: "memory://install-contrib-diff-plan",
      jobTemplateName: "flogo-runner",
      analysisPayload: {
        ...
        installPlanArtifact: {
          ...
          metadata: {
            result: {
              ...
              proposedRefs: [{ surface: "triggerRef", value: "#webhooktrigger", note: "Use this ref when creating a trigger instance." }],
              predictedChanges: {
                importsToAdd: [{ alias: "webhooktrigger", ref: "example.com/acme/webhook", action: "add" }],
                importsToUpdate: [],
                reusableRefs: [{ surface: "triggerRef", value: "#webhooktrigger", note: "Use this ref when creating a trigger instance." }],
                summaryLines: ["Add import alias \"webhooktrigger\" for ref \"example.com/acme/webhook\"."],
                noMutation: true
              },
              warnings: [],
    ...
    const diffArtifact = result.artifacts.find((artifact) => artifact.type === "contrib_install_diff_plan");
    expect(result.ok).toBe(true);
  ...
  it("executes helper-backed contribution update planning and publishes a conservative update-plan artifact", async () => {
    process.env.FLOGO_HELPER_BIN = await createHelperScript(
      JSON.stringify({
        result: {
          contributionKind: "trigger",
          ...
          limitations: ["Planning only; this slice does not mutate flogo.json."]
        }
      })
    );
    ...
    const result = await service.execute({
      ...
      envSecretRefs: {},
      timeoutSeconds: 60,
      artifactOutputUri: "memory://update-contrib-plan",
      jobTemplateName: "flogo-runner",
      analysisPayload: {
        packageArtifactId: "package-artifact-1",
        packageArtifact: {
    ...
    const updateArtifact = result.artifacts.find((artifact) => artifact.type === "contrib_update_plan");
    expect(result.ok).toBe(true);
    expect(updateArtifact).toBeDefined();
    expect((updateArtifact?.metadata as { result?: { updateReady?: boolean; matchQuality?: string; compatibility?: string } } | undefined)?.result?.updateReady).toBe(true);
    expect((updateArtifact?.metadata as { result?: { matchQuality?: string } } | undefined)?.result?.matchQuality).toBe("exact");
    expect((updateArtifact?.metadata as { result?: { compatibility?: string } } | undefined)?.result?.compatibility).toBe("compatible");
  });

  it("marks update-plan jobs as failed when the installed contribution match is ambiguous or missing", async () => {
    process.env.FLOGO_HELPER_BIN = await createHelperScript(
      JSON.stringify({
        result: {
          contributionKind: "trigger",
          ...
          predictedChanges: {
            ...
            refsToRemove: [],
            changedPaths: [],
            summaryLines: ["Update planning stopped because the target app does not currently install the requested contribution."],
            noMutation: true
          },
          warnings: ["No installed contribution match was detected; use install planning instead."],
          conflicts: [
            {
              kind: "no_installed_match",
              severity: "error",
              message: "Target app does not appear to have the contribution installed already."
            }
          ],
          diagnostics: [],
          recommendedNextAction: "Use install planning for new contributions.",
    ...
    const service = new RunnerExecutorService();
    const result = await service.execute({
      taskId: "task-update-contrib-plan-blocked",
      jobKind: "contrib_update_plan",
      stepType: "update_contrib_plan",
      analysisKind: "update_contrib_plan",
      snapshotUri: ".",
      ...
      envSecretRefs: {},
      timeoutSeconds: 60,
      artifactOutputUri: "memory://update-contrib-plan-blocked",
      jobTemplateName: "flogo-runner",
        analysisPayload: {
          packageResult: {
            source: "inline_result",
  ...
  it("executes update_contrib_apply through the helper and emits durable apply artifacts", async () => {
    process.env.FLOGO_HELPER_BIN = await createHelperScript(
      JSON.stringify({
        result: {
          contributionKind: "trigger",
          ...
          basedOnUpdateDiffPlan: {
            sourceArtifactId: "update-diff-artifact-1",
            updatePlanArtifactId: "update-plan-artifact-1",
            diffFingerprint: "diff-sha",
            appFingerprintBefore: "before-sha",
            appFingerprintPreview: "after-sha"
    ...
    const result = await service.execute({
      ...
      envSecretRefs: {},
      timeoutSeconds: 60,
      artifactOutputUri: "memory://update-contrib-apply",
      jobTemplateName: "flogo-runner",
      analysisPayload: {
        updateDiffPlanArtifactId: "update-diff-artifact-1",
        updateDiffPlanArtifact: {
          id: "update-diff-artifact-1",
          ...
          metadata: {
            result: {
              ...
              predictedChanges: {
                importsBefore: [],
                importsAfter: [{ alias: "webhooktrigger", ref: "example.com/acme/webhook", action: "predicted" }],
                importsToReplace: [{ alias: "webhooktrigger", ref: "example.com/acme/webhook", action: "replace_existing", existingAlias: "webhooktrigger", existingRef: "example.com/acme/webhook" }],
                importsToKeep: [],
                importsToAdd: [],
                importsToRemove: [],
                refsToReplace: [{ surface: "triggerRef", value: "#webhooktrigger", note: "Use this ref when creating a trigger instance." }],
                refsToKeep: [],
                refsToAdd: [],
                refsToRemove: [],
    ...
    const applyArtifact = result.artifacts.find((artifact) => artifact.type === "contrib_update_apply");
    const flogoJsonArtifact = result.artifacts.find((artifact) => artifact.type === "flogo_json");
    expect(result.ok).toBe(true);
    expect(applyArtifact).toBeDefined();
    expect(flogoJsonArtifact).toBeDefined();
  ...
  it("marks update apply jobs as failed when the approved diff preview is stale", async () => {
    process.env.FLOGO_HELPER_BIN = await createHelperScript(
      JSON.stringify({
        result: {
          contributionKind: "trigger",
          ...
          basedOnUpdateDiffPlan: {
            sourceArtifactId: "update-diff-artifact-2",
            updatePlanArtifactId: "update-plan-artifact-2"
          },
          appFingerprintBefore: "before-sha",
          isStale: true,
          staleReason: "The target app changed after the exact update diff preview was generated.",
          applied: false,
          applyReady: false,
          readiness: "low",
          warnings: ["The target app changed after the exact update diff preview was generated."],
          conflicts: [],
          limitations: [],
          changedPaths: ["imports"],
          appliedImports: [],
          appliedRefs: [],
          applySummary: ["Update apply stopped because flogo.json drifted from the exact diff basis."],
          canonicalBeforeJson: "{}",
          recommendedNextAction: "Regenerate the exact update diff preview before retrying apply."
        }
      })
    );
    ...
    const service = new RunnerExecutorService();
    const result = await service.execute({
      taskId: "task-update-contrib-apply-stale",
      jobKind: "contrib_update_apply",
      stepType: "update_contrib_apply",
      analysisKind: "update_contrib_apply",
      snapshotUri: ".",
      ...
      envSecretRefs: {},
      timeoutSeconds: 60,
      artifactOutputUri: "memory://update-contrib-apply-stale",
      jobTemplateName: "flogo-runner",
      analysisPayload: {
    ...
    expect(result.ok).toBe(false);
    expect(result.status).toBe("failed");
    expect(result.artifacts.some((artifact) => artifact.type === "contrib_update_apply")).toBe(true);
  });
  ...
  it("rejects malformed exact install diff-planning input before dispatching the helper", async () => {
    const service = new RunnerExecutorService();

    await expect(
      service.execute({
        taskId: "task-invalid-install-contrib-diff-plan",
        jobKind: "contrib_install_diff_plan",
        stepType: "install_contrib_diff_plan",
        analysisKind: "install_contrib_diff_plan",
        snapshotUri: ".",
        ...
        envSecretRefs: {},
        timeoutSeconds: 60,
        artifactOutputUri: "memory://invalid-install-contrib-diff-plan",
        jobTemplateName: "flogo-runner",
        analysisPayload: {},
        command: [],
        containerArgs: []
  ...
  it("rejects malformed update apply input before dispatching the helper", async () => {
    const service = new RunnerExecutorService();

    await expect(
      service.execute({
        taskId: "task-invalid-update-contrib-apply",
        jobKind: "contrib_update_apply",
        stepType: "update_contrib_apply",
        analysisKind: "update_contrib_apply",
        snapshotUri: ".",
        ...
        envSecretRefs: {},
        timeoutSeconds: 60,
        artifactOutputUri: "memory://invalid-update-contrib-apply",
        jobTemplateName: "flogo-runner",
        analysisPayload: {},
        command: [],
        containerArgs: []
      })
    ).rejects.toThrow(/Provide updateDiffPlanArtifactId, updateDiffPlanArtifact, or updateDiffPlanResult/);
  });

  it("rejects malformed contribution update-planning input before dispatching the helper", async () => {
    const service = new RunnerExecutorService();

    await expect(
      service.execute({
        taskId: "task-invalid-update-contrib-plan",
        jobKind: "contrib_update_plan",
        stepType: "update_contrib_plan",
        analysisKind: "update_contrib_plan",
        snapshotUri: ".",
        ...
        envSecretRefs: {},
        timeoutSeconds: 60,
        artifactOutputUri: "memory://invalid-update-contrib-plan",
        jobTemplateName: "flogo-runner",
        analysisPayload: {},
        command: [],
        containerArgs: []
