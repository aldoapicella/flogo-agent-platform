...
type actionScaffoldRequest struct {
...
type actionScaffoldBundle struct {
	Kind        string                 `json:"kind"`
	ModulePath  string                 `json:"modulePath"`
	PackageName string                 `json:"packageName"`
	BundleRoot  string                 `json:"bundleRoot"`
...
type actionScaffoldResult struct {
	Bundle     actionScaffoldBundle `json:"bundle"`
	Validation validationReport     `json:"validation"`
	Build      contribProofStep     `json:"build"`
	Test       contribProofStep     `json:"test"`
}

type actionScaffoldResponse struct {
	Result actionScaffoldResult `json:"result"`
}
...
type contributionInstallTarget struct {
...
type contributionInstallPlanRequest struct {
	Result          contributionScaffoldResult  `json:"result"`
	Package         *contributionPackageArchive `json:"package,omitempty"`
	Source          string                      `json:"source,omitempty"`
	SourceArtifact  string                      `json:"sourceArtifactId,omitempty"`
	TargetApp       contributionInstallTarget   `json:"targetApp,omitempty"`
	PreferredAlias  string                      `json:"preferredAlias,omitempty"`
	ReplaceExisting bool                        `json:"replaceExisting,omitempty"`
}

type contributionInstallImportEntry struct {
	Alias         string `json:"alias"`
	Ref           string `json:"ref"`
	Version       string `json:"version,omitempty"`
	Action        string `json:"action"`
	ExistingAlias string `json:"existingAlias,omitempty"`
	ExistingRef   string `json:"existingRef,omitempty"`
	Note          string `json:"note,omitempty"`
}

type contributionInstallRefEntry struct {
	Surface string `json:"surface"`
	Value   string `json:"value"`
	Note    string `json:"note,omitempty"`
}

type contributionInstallConflict struct {
	Kind          string `json:"kind"`
	Severity      string `json:"severity"`
	Message       string `json:"message"`
	ExistingAlias string `json:"existingAlias,omitempty"`
	ExistingRef   string `json:"existingRef,omitempty"`
	ProposedAlias string `json:"proposedAlias,omitempty"`
	ProposedRef   string `json:"proposedRef,omitempty"`
}

type contributionInstallPredictedChanges struct {
	...
	ReusableRefs    []contributionInstallRefEntry    `json:"reusableRefs"`
	SummaryLines    []string                         `json:"summaryLines"`
	NoMutation      bool                             `json:"noMutation"`
}

type contributionInstallPlanResult struct {
	...
	ProposedImports       []contributionInstallImportEntry    `json:"proposedImports"`
	ProposedRefs          []contributionInstallRefEntry       `json:"proposedRefs"`
	PredictedChanges      contributionInstallPredictedChanges `json:"predictedChanges"`
...
type contributionUpdateDiffPredictedChanges struct {
	ImportsBefore    []contributionInstallImportEntry `json:"importsBefore"`
	ImportsAfter     []contributionInstallImportEntry `json:"importsAfter"`
	ImportsToReplace []contributionInstallImportEntry `json:"importsToReplace"`
	ImportsToKeep    []contributionInstallImportEntry `json:"importsToKeep"`
...
type contributionUpdateDiffPlanBasis struct {
	SourceArtifact  string                     `json:"sourceArtifactId,omitempty"`
	AppFingerprint  string                     `json:"appFingerprint,omitempty"`
	PlanFingerprint string                     `json:"planFingerprint,omitempty"`
	TargetApp       *contributionInstallTarget `json:"targetApp,omitempty"`
...
type contributionUpdateDiffPlanResult struct {
	ContributionKind              string                                   `json:"contributionKind"`
	SourceContribution            contributionInstallDiffSource            `json:"sourceContribution"`
	DetectedInstalledContribution *contributionUpdateInstalledContribution `json:"detectedInstalledContribution,omitempty"`
	TargetApp                     contributionInstallTarget                `json:"targetApp"`
	BasedOnUpdatePlan             contributionUpdateDiffPlanBasis          `json:"basedOnUpdatePlan"`
	AppFingerprintBefore          string                                   `json:"appFingerprintBefore"`
	AppFingerprintAfter           string                                   `json:"appFingerprintAfter,omitempty"`
	UpdatePlanFingerprint         string                                   `json:"updatePlanFingerprint,omitempty"`
	IsStale                       bool                                     `json:"isStale"`
	StaleReason                   string                                   `json:"staleReason,omitempty"`
	PreviewAvailable              bool                                     `json:"previewAvailable"`
	UpdateReady                   bool                                     `json:"updateReady"`
	...
	Conflicts                     []contributionInstallConflict            `json:"conflicts"`
	Limitations                   []string                                 `json:"limitations"`
	PredictedChanges              contributionUpdateDiffPredictedChanges   `json:"predictedChanges"`
	DiffSummary                   []string                                 `json:"diffSummary"`
	CanonicalBeforeJSON           string                                   `json:"canonicalBeforeJson"`
	CanonicalAfterJSON            string                                   `json:"canonicalAfterJson,omitempty"`
	RecommendedNextAction         string                                   `json:"recommendedNextAction"`
...
type contributionUpdateDiffPlanResponse struct {
	Result contributionUpdateDiffPlanResult `json:"result"`
}
...
type contributionUpdateApplyRequest struct {
	UpdateDiffPlan        contributionUpdateDiffPlanResult `json:"updateDiffPlan"`
	UpdateDiffArtifact    string                           `json:"updateDiffPlanArtifactId,omitempty"`
	SourceUpdatePlanEntry string                           `json:"sourceUpdatePlanArtifactId,omitempty"`
	TargetApp             contributionInstallTarget        `json:"targetApp,omitempty"`
}

type contributionUpdateApplyBasis struct {
	SourceArtifact        string                     `json:"sourceArtifactId,omitempty"`
	UpdatePlanArtifact    string                     `json:"updatePlanArtifactId,omitempty"`
	DiffFingerprint       string                     `json:"diffFingerprint,omitempty"`
	AppFingerprintBefore  string                     `json:"appFingerprintBefore,omitempty"`
...
type contributionUpdateApplyResult struct {
	ContributionKind              string                                   `json:"contributionKind"`
	SourceContribution            contributionInstallDiffSource            `json:"sourceContribution"`
	DetectedInstalledContribution *contributionUpdateInstalledContribution `json:"detectedInstalledContribution,omitempty"`
	TargetApp                     contributionInstallTarget                `json:"targetApp"`
	BasedOnUpdateDiff             contributionUpdateApplyBasis             `json:"basedOnUpdateDiffPlan"`
	AppFingerprintBefore          string                                   `json:"appFingerprintBefore"`
	AppFingerprintAfter           string                                   `json:"appFingerprintAfter,omitempty"`
	IsStale                       bool                                     `json:"isStale"`
	StaleReason                   string                                   `json:"staleReason,omitempty"`
...
type contributionUpdateApplyResponse struct {
	Result contributionUpdateApplyResult `json:"result"`
}
...
func main() {
	...
	if command == "contrib scaffold-action" {
		requestPath := lookupFlag("--request")
		if requestPath == "" {
			fail("missing required --request flag")
		}
		encode(scaffoldAction(loadActionScaffoldRequest(requestPath)))
		return
	}
	...
	if command == "contrib install-plan" {
	...
	if command == "contrib update-diff-plan" {
		...
		encode(planContributionUpdateDiff(loadApp(appPath), appPath, loadContributionUpdateDiffPlanRequest(requestPath)))
		return
	}
	...
	if command == "contrib update-apply" {
		appPath := lookupFlag("--app")
		if appPath == "" {
			fail("missing required --app flag")
		}
		...
		encode(applyContributionUpdateDiff(loadApp(appPath), appPath, loadContributionUpdateApplyRequest(requestPath)))
		return
	}
...
func loadActionScaffoldRequest(inputPath string) actionScaffoldRequest {
	contents, err := os.ReadFile(inputPath)
	if err != nil {
		fail(err.Error())
	}

	var request actionScaffoldRequest
	if err := json.Unmarshal(contents, &request); err != nil {
		fail(err.Error())
	}
	if request.Version == "" {
...
func loadContributionInstallPlanRequest(inputPath string) contributionInstallPlanRequest {
	contents, err := os.ReadFile(inputPath)
	if err != nil {
		fail(err.Error())
	}

	var request contributionInstallPlanRequest
	if err := json.Unmarshal(contents, &request); err != nil {
		fail(err.Error())
	}
...
func loadContributionUpdateDiffPlanRequest(inputPath string) contributionUpdateDiffPlanRequest {
	...
	var request contributionUpdateDiffPlanRequest
	if err := json.Unmarshal(contents, &request); err != nil {
		fail(err.Error())
	}
...
func loadContributionUpdateApplyRequest(inputPath string) contributionUpdateApplyRequest {
	contents, err := os.ReadFile(inputPath)
	if err != nil {
		fail(err.Error())
	}

	var request contributionUpdateApplyRequest
	if err := json.Unmarshal(contents, &request); err != nil {
		fail(err.Error())
	}
...
func scaffoldAction(request actionScaffoldRequest) actionScaffoldResponse {
	if err := validateActionScaffoldRequest(request); err != nil {
		fail(err.Error())
	}

	packageName := sanitizePackageName(valueOrFallback(request.PackageName, request.ActionName))
	descriptor := buildScaffoldedActionDescriptor(request, packageName)
	bundleRoot, err := os.MkdirTemp("", fmt.Sprintf("flogo-action-%s-", packageName))
	if err != nil {
		fail(err.Error())
	}

	files, err := writeScaffoldedActionFiles(bundleRoot, request, descriptor, packageName)
	if err != nil {
		fail(err.Error())
	}
	...
	validation, buildProof, testProof := runContributionScaffoldProof(
		bundleRoot,
		"Action",
		"flogo.contrib.action",
	)

	return actionScaffoldResponse{
		Result: actionScaffoldResult{
			Bundle: actionScaffoldBundle{
				Kind:        "action",
				ModulePath:  strings.TrimSpace(request.ModulePath),
				PackageName: packageName,
				BundleRoot:  bundleRoot,
...
func validateContributionInstallPlanRequest(request contributionInstallPlanRequest) error {
	if err := validateContributionScaffoldResult(request.Result); err != nil {
		return err
	}
	if request.Package != nil && strings.TrimSpace(request.Package.Format) != "" && strings.TrimSpace(request.Package.Format) != "zip" {
		return fmt.Errorf("unsupported contribution package format %q", request.Package.Format)
	}
	return nil
}
...
func validateContributionUpdateDiffPlanRequest(request contributionUpdateDiffPlanRequest) error {
	...
	if strings.TrimSpace(request.UpdatePlan.SelectedAlias) == "" {
		return fmt.Errorf("updatePlan.selectedAlias is required")
	}
	if strings.TrimSpace(request.UpdatePlan.ModulePath) == "" {
		return fmt.Errorf("updatePlan.modulePath is required")
	}
	return nil
}
...
func validateContributionUpdateApplyRequest(request contributionUpdateApplyRequest) error {
	diff := request.UpdateDiffPlan
	switch strings.TrimSpace(diff.ContributionKind) {
	case "activity", "action", "trigger":
	default:
		return fmt.Errorf("updateDiffPlan.contributionKind %q is unsupported", diff.ContributionKind)
	}
	if strings.TrimSpace(diff.SourceContribution.ModulePath) == "" {
		return fmt.Errorf("updateDiffPlan.sourceContribution.modulePath is required")
	}
	if strings.TrimSpace(diff.SourceContribution.SelectedAlias) == "" {
		return fmt.Errorf("updateDiffPlan.sourceContribution.selectedAlias is required")
	}
	if strings.TrimSpace(diff.AppFingerprintBefore) == "" {
		return fmt.Errorf("updateDiffPlan.appFingerprintBefore is required")
	}
	if strings.TrimSpace(diff.CanonicalBeforeJSON) == "" {
		return fmt.Errorf("updateDiffPlan.canonicalBeforeJson is required")
	}
	if len(diff.PredictedChanges.ChangedPaths) > 0 && strings.TrimSpace(diff.CanonicalAfterJSON) == "" {
		return fmt.Errorf("updateDiffPlan.canonicalAfterJson is required when the preview includes canonical changes")
	}
	if !diff.PreviewAvailable && !diff.PredictedChanges.NoMutation {
		return fmt.Errorf("updateDiffPlan must be previewAvailable before apply")
	}
	return nil
}
...
func normalizeContributionInstallSource(source string, sourceArtifact string, hasPackage bool) string {
	normalized := strings.TrimSpace(source)
	switch normalized {
	case "inline_result", "bundle_artifact", "inline_package", "package_artifact":
		return normalized
	}
	if hasPackage {
		if strings.TrimSpace(sourceArtifact) != "" {
...
func contributionUpdateDiffPlanFingerprint(result contributionUpdateDiffPlanResult) string {
	return hashProjection(map[string]any{
		"contributionKind": result.ContributionKind,
		"sourceContribution": map[string]any{
			"kind":           result.SourceContribution.Kind,
...
func removeImportEntry(entries []flogoImport, alias string) ([]flogoImport, flogoImport, bool) {
	normalizedAlias := normalizeAlias(alias)
	for idx, entry := range entries {
		if normalizeAlias(entry.Alias) == normalizedAlias {
			cloned := append([]flogoImport{}, entries[:idx]...)
			cloned = append(cloned, entries[idx+1:]...)
			return cloned, entry, true
		}
	}
	return entries, flogoImport{}, false
}
...
func updateDiffSourceFromPlan(plan contributionUpdatePlanResult, artifactID string) contributionInstallDiffSource {
	sourceArtifact := strings.TrimSpace(artifactID)
	if sourceArtifact == "" {
		sourceArtifact = strings.TrimSpace(plan.SourceArtifact)
	}
...
func planContributionUpdateDiff(app flogoApp, appPath string, request contributionUpdateDiffPlanRequest) contributionUpdateDiffPlanResponse {
	if err := validateContributionUpdateDiffPlanRequest(request); err != nil {
		fail(err.Error())
	}
	...
	appFingerprintBefore := appCanonicalFingerprint(app)
	computedPlanFingerprint := contributionUpdatePlanFingerprint(plan)
	warnings := cloneStringSlice(plan.Warnings)
	conflicts := cloneContributionInstallConflicts(plan.Conflicts)
	limitations := dedupeStrings(append(cloneStringSlice(plan.Limitations),
		"Diff preview only; this slice computes a reviewable canonical flogo.json preview without writing app files.",
		"Exact preview is currently limited to canonical import mutations and does not rewrite usage sites beyond the alias/ref decisions already proven by update planning.",
	))
	diffSummary := []string{}

	sourceContribution := updateDiffSourceFromPlan(plan, request.UpdatePlanArtifact)
	basis := contributionUpdateDiffPlanBasis{
		SourceArtifact:  strings.TrimSpace(request.UpdatePlanArtifact),
		AppFingerprint:  strings.TrimSpace(plan.AppFingerprint),
		PlanFingerprint: strings.TrimSpace(plan.PlanFingerprint),
	}
	if basis.SourceArtifact == "" {
	...
	result := contributionUpdateDiffPlanResult{
		ContributionKind:              plan.ContributionKind,
		SourceContribution:            sourceContribution,
		DetectedInstalledContribution: plan.DetectedInstalledContribution,
		TargetApp:                     targetApp,
		BasedOnUpdatePlan:             basis,
		AppFingerprintBefore:          appFingerprintBefore,
		UpdatePlanFingerprint:         computedPlanFingerprint,
		IsStale:                       false,
		PreviewAvailable:              false,
		...
		PredictedChanges: contributionUpdateDiffPredictedChanges{
			ImportsBefore:    []contributionInstallImportEntry{},
			ImportsAfter:     []contributionInstallImportEntry{},
			ImportsToReplace: cloneContributionInstallImportEntries(plan.PredictedChanges.ImportsToReplace),
			ImportsToKeep:    cloneContributionInstallImportEntries(plan.PredictedChanges.ImportsToKeep),
			ImportsToAdd:     cloneContributionInstallImportEntries(plan.PredictedChanges.ImportsToAdd),
			ImportsToRemove:  cloneContributionInstallImportEntries(plan.PredictedChanges.ImportsToRemove),
			RefsToReplace:    cloneContributionInstallRefEntries(plan.PredictedChanges.RefsToReplace),
			RefsToKeep:       cloneContributionInstallRefEntries(plan.PredictedChanges.RefsToKeep),
			RefsToAdd:        cloneContributionInstallRefEntries(plan.PredictedChanges.RefsToAdd),
			RefsToRemove:     cloneContributionInstallRefEntries(plan.PredictedChanges.RefsToRemove),
			StructuralChange: []string{},
			ChangedPaths:     []string{},
			DiffEntries:      []contributionInstallDiffChange{},
			NoMutation:       true,
		...
		DiffSummary:           []string{},
		CanonicalBeforeJSON:   canonicalJSONString(app.Raw),
		RecommendedNextAction: "Regenerate the contribution update plan before producing an exact canonical diff preview.",
	}
	...
	if !sameInstallTarget(targetApp, plan.TargetApp) {
		result.IsStale = true
		result.StaleReason = "The update plan target app no longer matches the requested target app context."
		result.UpdateReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, result.StaleReason)
		result.DiffSummary = append(result.DiffSummary, "Update diff preview stopped because the target app context changed since update planning.")
		return contributionUpdateDiffPlanResponse{Result: result}
	}

	if strings.TrimSpace(plan.AppFingerprint) == "" {
		result.IsStale = true
		result.StaleReason = "The update plan does not include an app fingerprint, so freshness cannot be verified."
		result.UpdateReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, result.StaleReason)
		result.DiffSummary = append(result.DiffSummary, "Update diff preview requires a fresh update plan that records the target app fingerprint.")
		return contributionUpdateDiffPlanResponse{Result: result}
	}

	if strings.TrimSpace(plan.AppFingerprint) != appFingerprintBefore {
		result.IsStale = true
		result.StaleReason = "The target app changed after update planning; regenerate the update plan before reviewing an exact diff."
		result.UpdateReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, result.StaleReason)
		result.DiffSummary = append(result.DiffSummary, "Update diff preview stopped because flogo.json drifted from the update-plan basis.")
		return contributionUpdateDiffPlanResponse{Result: result}
	}

	if strings.TrimSpace(plan.PlanFingerprint) != "" && strings.TrimSpace(plan.PlanFingerprint) != computedPlanFingerprint {
		result.IsStale = true
		result.StaleReason = "The update plan payload no longer matches its recorded fingerprint."
		result.UpdateReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, result.StaleReason)
		result.DiffSummary = append(result.DiffSummary, "Update diff preview stopped because the update plan payload appears to have changed since planning.")
		return contributionUpdateDiffPlanResponse{Result: result}
	}

	if !plan.UpdateReady || (plan.MatchQuality != "exact" && plan.MatchQuality != "likely") || plan.Compatibility != "compatible" || plan.DetectedInstalledContribution == nil {
		result.PreviewAvailable = false
		result.UpdateReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		if plan.DetectedInstalledContribution == nil {
			result.Warnings = append(result.Warnings, "Update diff preview requires a confident installed-contribution match from the saved update plan.")
		}
		if len(result.DiffSummary) == 0 {
			result.DiffSummary = append(result.DiffSummary, "Update diff preview stopped because the saved update plan is not precise enough to materialize an exact canonical preview safely.")
		}
		result.RecommendedNextAction = "Regenerate or refine the update plan until the installed contribution match and compatibility are strong enough for exact diff preview."
		return contributionUpdateDiffPlanResponse{Result: result}
	}

	afterImports := append([]flogoImport{}, app.Imports...)
	for _, proposed := range plan.PredictedChanges.ImportsToReplace {
		existing, ok := findPreviewImport(afterImports, proposed)
		if !ok {
			result.PreviewAvailable = false
			result.UpdateReady = false
			result.Readiness = lowerInstallReadiness(result.Readiness, "low")
			result.Warnings = append(result.Warnings, fmt.Sprintf("Update diff preview could not find alias %q to replace in the target app.", valueOrFallback(proposed.Alias, proposed.ExistingAlias)))
			result.DiffSummary = append(result.DiffSummary, fmt.Sprintf("Stopped update diff preview because alias %q could not be found for replacement.", valueOrFallback(proposed.Alias, proposed.ExistingAlias)))
			result.RecommendedNextAction = "Regenerate the update plan against the current app state before reviewing an exact diff preview."
			return contributionUpdateDiffPlanResponse{Result: result}
		}
		nextEntry := flogoImport{
			Alias:   valueOrFallback(proposed.Alias, existing.Alias),
			Ref:     valueOrFallback(proposed.Ref, existing.Ref),
			Version: valueOrFallback(proposed.Version, existing.Version),
		}
		if normalizeAlias(existing.Alias) != normalizeAlias(nextEntry.Alias) {
			result.PreviewAvailable = false
			result.UpdateReady = false
			result.Readiness = lowerInstallReadiness(result.Readiness, "low")
			result.Warnings = append(result.Warnings, fmt.Sprintf("Update diff preview will not rewrite alias %q to %q without a later update-apply workflow.", existing.Alias, nextEntry.Alias))
			result.DiffSummary = append(result.DiffSummary, fmt.Sprintf("Stopped update diff preview because alias %q would need to change to %q.", existing.Alias, nextEntry.Alias))
			result.RecommendedNextAction = "Keep alias rewrites deferred until a later review-gated update/apply slice."
			return contributionUpdateDiffPlanResponse{Result: result}
		}
		updated, replaced := replaceImportEntry(afterImports, existing.Alias, nextEntry)
		if !replaced {
			...
			result.UpdateReady = false
			result.Readiness = lowerInstallReadiness(result.Readiness, "low")
			result.Warnings = append(result.Warnings, fmt.Sprintf("Update diff preview could not replace alias %q in the target app.", existing.Alias))
			result.DiffSummary = append(result.DiffSummary, fmt.Sprintf("Stopped update diff preview because alias %q could not be replaced.", existing.Alias))
			result.RecommendedNextAction = "Regenerate the update plan against the current app state before reviewing an exact diff preview."
			return contributionUpdateDiffPlanResponse{Result: result}
		}
		afterImports = updated
		result.PredictedChanges.ChangedPaths = appendUniqueString(result.PredictedChanges.ChangedPaths, "imports")
		summary := fmt.Sprintf("Update import alias %q from ref %q to %q.", existing.Alias, existing.Ref, nextEntry.Ref)
		if existing.Ref == nextEntry.Ref && existing.Version != nextEntry.Version {
			summary = fmt.Sprintf("Update import alias %q version from %q to %q.", existing.Alias, valueOrFallback(existing.Version, "unset"), valueOrFallback(nextEntry.Version, "unset"))
		}
		result.PredictedChanges.StructuralChange = append(result.PredictedChanges.StructuralChange, summary)
		result.PredictedChanges.DiffEntries = append(result.PredictedChanges.DiffEntries, contributionInstallDiffChange{
			Path:       "imports",
			ChangeType: "update",
			Summary:    summary,
	...
	for _, proposed := range plan.PredictedChanges.ImportsToKeep {
		existing, ok := findPreviewImport(afterImports, proposed)
		if !ok {
			result.PreviewAvailable = false
			result.UpdateReady = false
			result.Readiness = lowerInstallReadiness(result.Readiness, "low")
			result.Warnings = append(result.Warnings, fmt.Sprintf("Update diff preview could not confirm existing alias %q for a no-op preview.", valueOrFallback(proposed.Alias, proposed.ExistingAlias)))
			result.DiffSummary = append(result.DiffSummary, fmt.Sprintf("Stopped update diff preview because alias %q no longer matches the saved update-plan basis.", valueOrFallback(proposed.Alias, proposed.ExistingAlias)))
			result.RecommendedNextAction = "Regenerate the update plan against the current app state before reviewing an exact diff preview."
			return contributionUpdateDiffPlanResponse{Result: result}
		}
		summary := fmt.Sprintf("No canonical import mutation is required because alias %q already matches the requested contribution metadata.", existing.Alias)
		result.PredictedChanges.DiffEntries = append(result.PredictedChanges.DiffEntries, contributionInstallDiffChange{
			Path:       "imports",
			ChangeType: "reuse",
			Summary:    summary,
	...
	for _, proposed := range plan.PredictedChanges.ImportsToAdd {
		nextEntry := flogoImport{
			Alias:   strings.TrimSpace(proposed.Alias),
			Ref:     strings.TrimSpace(proposed.Ref),
			Version: strings.TrimSpace(proposed.Version),
		...
		if existing, ok := findPreviewImport(afterImports, contributionInstallImportEntry{Alias: proposed.Alias}); ok && strings.TrimSpace(existing.Ref) != strings.TrimSpace(proposed.Ref) {
			...
			result.Warnings = append(result.Warnings, "Update diff preview found a new alias conflict while applying the saved update plan.")
			result.DiffSummary = append(result.DiffSummary, fmt.Sprintf("Stopped update diff preview because alias %q now points to %q.", proposed.Alias, existing.Ref))
			result.RecommendedNextAction = "Regenerate the update plan and resolve the alias conflict before reviewing an exact diff preview."
			return contributionUpdateDiffPlanResponse{Result: result}
		}
		afterImports = append(afterImports, nextEntry)
		result.PredictedChanges.ChangedPaths = appendUniqueString(result.PredictedChanges.ChangedPaths, "imports")
	...
	for _, proposed := range plan.PredictedChanges.ImportsToRemove {
		existing, ok := findPreviewImport(afterImports, proposed)
		if !ok {
			result.PreviewAvailable = false
			result.UpdateReady = false
			result.Readiness = lowerInstallReadiness(result.Readiness, "low")
			result.Warnings = append(result.Warnings, fmt.Sprintf("Update diff preview could not find alias %q to remove in the target app.", valueOrFallback(proposed.Alias, proposed.ExistingAlias)))
			result.DiffSummary = append(result.DiffSummary, fmt.Sprintf("Stopped update diff preview because alias %q could not be removed.", valueOrFallback(proposed.Alias, proposed.ExistingAlias)))
			result.RecommendedNextAction = "Regenerate the update plan against the current app state before reviewing an exact diff preview."
			return contributionUpdateDiffPlanResponse{Result: result}
		}
		updated, removed, ok := removeImportEntry(afterImports, existing.Alias)
		if !ok {
			result.PreviewAvailable = false
			result.UpdateReady = false
			result.Readiness = lowerInstallReadiness(result.Readiness, "low")
			result.Warnings = append(result.Warnings, fmt.Sprintf("Update diff preview could not remove alias %q from the target app.", existing.Alias))
			result.DiffSummary = append(result.DiffSummary, fmt.Sprintf("Stopped update diff preview because alias %q could not be removed.", existing.Alias))
			result.RecommendedNextAction = "Regenerate the update plan against the current app state before reviewing an exact diff preview."
			return contributionUpdateDiffPlanResponse{Result: result}
		}
		afterImports = updated
		result.PredictedChanges.ChangedPaths = appendUniqueString(result.PredictedChanges.ChangedPaths, "imports")
		summary := fmt.Sprintf("Remove import alias %q for ref %q.", removed.Alias, removed.Ref)
		result.PredictedChanges.StructuralChange = append(result.PredictedChanges.StructuralChange, summary)
		result.PredictedChanges.DiffEntries = append(result.PredictedChanges.DiffEntries, contributionInstallDiffChange{
			Path:       "imports",
			ChangeType: "update",
	...
	if len(result.PredictedChanges.ChangedPaths) == 0 {
		diffSummary = append(diffSummary, "No canonical flogo.json import mutation is required for this update plan.")
	}
	...
	result.PreviewAvailable = true
	if len(result.PredictedChanges.ChangedPaths) > 0 {
		result.RecommendedNextAction = "Review the exact canonical update diff and keep update/apply gated to a later explicit mutation workflow."
	} else {
		result.RecommendedNextAction = "Review the no-op canonical update diff and keep update/apply gated to a later explicit mutation workflow."
	}
...
func applyContributionUpdateDiff(app flogoApp, appPath string, request contributionUpdateApplyRequest) contributionUpdateApplyResponse {
	if err := validateContributionUpdateApplyRequest(request); err != nil {
		fail(err.Error())
	}
	...
	currentCanonicalBefore := canonicalJSONString(app.Raw)
	currentFingerprint := appCanonicalFingerprint(app)
	diffFingerprint := contributionUpdateDiffPlanFingerprint(diff)
	warnings := cloneStringSlice(diff.Warnings)
	conflicts := cloneContributionInstallConflicts(diff.Conflicts)
	limitations := dedupeStrings(append(cloneStringSlice(diff.Limitations),
		"Apply only materializes the previously previewed canonical flogo.json output for the reviewed update; it does not re-plan update decisions.",
		"Contribution update apply currently mutates canonical imports only when the exact update diff preview carried a precise canonical-after document.",
	))
	changedPaths := dedupeStrings(cloneStringSlice(diff.PredictedChanges.ChangedPaths))
	appliedImports := append(
		cloneContributionInstallImportEntries(diff.PredictedChanges.ImportsToReplace),
		cloneContributionInstallImportEntries(diff.PredictedChanges.ImportsToAdd)...,
	)
	appliedRefs := dedupeContributionInstallRefEntries(append(
		cloneContributionInstallRefEntries(diff.PredictedChanges.RefsToReplace),
		append(
			cloneContributionInstallRefEntries(diff.PredictedChanges.RefsToAdd),
			cloneContributionInstallRefEntries(diff.PredictedChanges.RefsToKeep)...,
		)...,
	))
	applySummary := cloneStringSlice(diff.DiffSummary)

	basisTarget := copyInstallTarget(diff.TargetApp)
	result := contributionUpdateApplyResult{
		ContributionKind:              diff.ContributionKind,
		SourceContribution:            diff.SourceContribution,
		DetectedInstalledContribution: diff.DetectedInstalledContribution,
		TargetApp:                     targetApp,
		BasedOnUpdateDiff: contributionUpdateApplyBasis{
			SourceArtifact:        strings.TrimSpace(request.UpdateDiffArtifact),
			UpdatePlanArtifact:    valueOrFallback(strings.TrimSpace(request.SourceUpdatePlanEntry), strings.TrimSpace(diff.BasedOnUpdatePlan.SourceArtifact)),
			DiffFingerprint:       diffFingerprint,
			AppFingerprintBefore:  strings.TrimSpace(diff.AppFingerprintBefore),
			AppFingerprintPreview: strings.TrimSpace(diff.AppFingerprintAfter),
			TargetApp:             &basisTarget,
		...
		ApplySummary:          applySummary,
		CanonicalBeforeJSON:   currentCanonicalBefore,
		RecommendedNextAction: "Regenerate the exact update diff preview before attempting contribution update apply.",
	}

	if !sameInstallTarget(targetApp, diff.TargetApp) {
		result.IsStale = true
		result.StaleReason = "The update diff preview target app no longer matches the requested target app context."
		result.ApplyReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, result.StaleReason)
		result.ApplySummary = append(result.ApplySummary, "Update apply stopped because the target app context changed since exact diff preview.")
		return contributionUpdateApplyResponse{Result: result}
	}

	if diff.IsStale {
		result.IsStale = true
		result.StaleReason = valueOrFallback(strings.TrimSpace(diff.StaleReason), "The exact update diff preview is already stale.")
		result.ApplyReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, result.StaleReason)
		result.ApplySummary = append(result.ApplySummary, "Update apply stopped because the saved exact diff preview is stale.")
		return contributionUpdateApplyResponse{Result: result}
	}
	...
	if !diff.PreviewAvailable {
		result.ApplyReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, "Update apply requires a previewAvailable exact diff plan.")
		result.ApplySummary = append(result.ApplySummary, "Update apply stopped because the saved diff preview was not materialized.")
		return contributionUpdateApplyResponse{Result: result}
	}
	...
	if !diff.UpdateReady {
		result.ApplyReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, "Update apply requires an update-ready exact diff preview.")
		result.ApplySummary = append(result.ApplySummary, "Update apply stopped because the saved exact diff preview was not marked update-ready.")
		return contributionUpdateApplyResponse{Result: result}
	}

	if strings.TrimSpace(diff.AppFingerprintBefore) == "" {
		result.IsStale = true
		result.StaleReason = "The exact update diff preview does not record the target app fingerprint."
		result.ApplyReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, result.StaleReason)
		result.ApplySummary = append(result.ApplySummary, "Update apply requires a fresh exact diff preview that records the target app fingerprint.")
		return contributionUpdateApplyResponse{Result: result}
	}

	if strings.TrimSpace(diff.AppFingerprintBefore) != currentFingerprint {
		result.IsStale = true
		result.StaleReason = "The target app changed after the exact update diff preview was generated."
		result.ApplyReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, result.StaleReason)
		result.ApplySummary = append(result.ApplySummary, "Update apply stopped because flogo.json drifted from the exact diff basis.")
		return contributionUpdateApplyResponse{Result: result}
	}
	...
	if strings.TrimSpace(diff.CanonicalBeforeJSON) == "" {
		result.ApplyReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, "Update apply requires the exact diff preview to record the canonical before document.")
		result.ApplySummary = append(result.ApplySummary, "Update apply stopped because the saved diff preview omitted the canonical before document.")
		return contributionUpdateApplyResponse{Result: result}
	}

	if currentCanonicalBefore != strings.TrimSpace(diff.CanonicalBeforeJSON) {
		result.IsStale = true
		result.StaleReason = "The target app canonical document no longer matches the exact update diff preview basis."
		result.ApplyReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, result.StaleReason)
		result.ApplySummary = append(result.ApplySummary, "Update apply stopped because the canonical before document drifted since the exact diff preview.")
		return contributionUpdateApplyResponse{Result: result}
	}
	...
	if len(changedPaths) > 0 && strings.TrimSpace(diff.CanonicalAfterJSON) == "" {
		result.ApplyReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, "Update apply requires the exact diff preview to include the canonical after document.")
		result.ApplySummary = append(result.ApplySummary, "Update apply stopped because the exact after document was missing from the saved diff preview.")
		return contributionUpdateApplyResponse{Result: result}
	}
	...
	if err != nil {
		result.ApplyReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, fmt.Sprintf("Update apply could not decode the exact canonical after document: %v", err))
		result.ApplySummary = append(result.ApplySummary, "Update apply stopped because the saved exact after document could not be decoded.")
		return contributionUpdateApplyResponse{Result: result}
	}
	...
	if strings.TrimSpace(diff.AppFingerprintAfter) != "" && strings.TrimSpace(diff.AppFingerprintAfter) != afterFingerprint {
		result.ApplyReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, "Update apply stopped because the exact after document no longer matches the saved diff preview fingerprint.")
		result.ApplySummary = append(result.ApplySummary, "Update apply stopped because the saved exact after document failed fingerprint verification.")
		return contributionUpdateApplyResponse{Result: result}
	}
	...
	if len(changedPaths) == 0 && currentCanonicalBefore != canonicalAfterJSON {
		result.ApplyReady = false
		result.Readiness = lowerInstallReadiness(result.Readiness, "low")
		result.Warnings = append(result.Warnings, "Update apply stopped because the saved diff preview claims no canonical mutation but carries a different after document.")
		result.ApplySummary = append(result.ApplySummary, "Update apply stopped because the no-op diff preview carried an inconsistent after document.")
		return contributionUpdateApplyResponse{Result: result}
	}
	...
	if currentFingerprint != afterFingerprint {
		if err := os.WriteFile(targetApp.AppPath, []byte(canonicalAfterJSON), 0o644); err != nil {
			result.ApplyReady = false
			result.Readiness = lowerInstallReadiness(result.Readiness, "low")
			result.Warnings = append(result.Warnings, fmt.Sprintf("Update apply could not write the updated canonical app: %v", err))
			result.ApplySummary = append(result.ApplySummary, "Update apply stopped because the updated canonical flogo.json could not be written to the target app path.")
			result.RecommendedNextAction = "Fix target app write permissions or workspace path issues, then regenerate the exact update diff preview before retrying apply."
			return contributionUpdateApplyResponse{Result: result}
		}
		result.Applied = true
		result.ApplySummary = append(result.ApplySummary, "Applied the exact canonical flogo.json mutation preview to the target app.")
		result.RecommendedNextAction = "Review the applied canonical flogo.json output and run follow-up validation before broader update workflows."
	} else {
		result.Applied = false
		result.ApplySummary = append(result.ApplySummary, "No canonical mutation was required by the saved exact diff preview.")
		result.RecommendedNextAction = "No canonical mutation was required; persist the unchanged flogo.json only if the wider workflow expects an explicit apply artifact."
...
func contributionDescriptorRef(bundle contributionScaffoldBundle) string {
	if strings.TrimSpace(bundle.Descriptor.Ref) != "" {
		return strings.TrimSpace(bundle.Descriptor.Ref)
	}
	return strings.TrimSpace(bundle.ModulePath)
}

func contributionDescriptorVersion(bundle contributionScaffoldBundle) string {
	return strings.TrimSpace(bundle.Descriptor.Version)
}

func contributionInstallSurface(kind string) (string, string) {
	switch strings.TrimSpace(kind) {
	case "activity":
		return "activityRef", "Use this ref in task activityRef fields."
	case "action":
		return "actionRef", "Use this ref in trigger handler action refs."
	case "trigger":
		return "triggerRef", "Use this ref when creating a trigger instance."
	default:
		return "activityRef", "Use this ref where the target app can reference the contribution."
	}
}

func contributionInstallAlias(bundle contributionScaffoldBundle, preferredAlias string) string {
	switch {
	case strings.TrimSpace(preferredAlias) != "":
		return sanitizePackageName(preferredAlias)
	case strings.TrimSpace(bundle.Descriptor.Alias) != "":
		return sanitizePackageName(bundle.Descriptor.Alias)
	case strings.TrimSpace(bundle.PackageName) != "":
		return sanitizePackageName(bundle.PackageName)
	default:
		return sanitizePackageName(inferAlias(contributionDescriptorRef(bundle)))
	}
}

func installReadinessRank(readiness string) int {
	switch readiness {
	case "high":
		return 3
	case "medium":
...
func lowerInstallReadiness(current string, next string) string {
	if installReadinessRank(next) < installReadinessRank(current) {
		return next
	}
	return current
}

func findImportByAlias(app flogoApp, alias string) (flogoImport, bool) {
	normalized := normalizeAlias(alias)
	for _, entry := range app.Imports {
		if normalizeAlias(entry.Alias) == normalized {
			return entry, true
		}
	}
	return flogoImport{}, false
...
func findImportsByRef(app flogoApp, ref string) []flogoImport {
	matches := []flogoImport{}
	for _, entry := range app.Imports {
		if strings.TrimSpace(entry.Ref) == strings.TrimSpace(ref) {
...
func containsInstallConflict(conflicts []contributionInstallConflict, kind string) bool {
...
func planContributionInstall(app flogoApp, appPath string, request contributionInstallPlanRequest) contributionInstallPlanResponse {
	if err := validateContributionInstallPlanRequest(request); err != nil {
		fail(err.Error())
	}
	...
	if descriptorRef == "" {
		fail("contribution install planning requires bundle.descriptor.ref or bundle.modulePath")
	}

	selectedAlias := contributionInstallAlias(bundle, request.PreferredAlias)
	...
	proposedImport := contributionInstallImportEntry{
		Alias:  selectedAlias,
		Ref:    descriptorRef,
		Action: "add",
	}
	if version != "" {
		proposedImport.Version = version
	}

	inventory := buildContributionInventory(app, appPath)
	if existingEntry, ok := findInventoryEntry(inventory, app, descriptorRef); ok {
		if existingType := strings.TrimSpace(existingEntry.Type); existingType != "" && existingType != bundle.Kind {
			conflicts = append(conflicts, contributionInstallConflict{
				Kind:          "type_conflict",
				Severity:      "error",
				Message:       fmt.Sprintf("Target app already resolves %q as a %s, not a %s.", descriptorRef, existingType, bundle.Kind),
				ProposedAlias: selectedAlias,
				ProposedRef:   descriptorRef,
			})
			diagnostics = append(diagnostics, diagnostic{
				Code:     "flogo.contrib.install_plan.type_conflict",
				Message:  fmt.Sprintf("Resolved ref %q already maps to %s metadata in the target app.", descriptorRef, existingType),
				Severity: "error",
				Path:     "imports." + selectedAlias,
			})
			installReady = false
	...
	if existingAlias, ok := findImportByAlias(app, selectedAlias); ok {
		proposedImport.ExistingAlias = existingAlias.Alias
		proposedImport.ExistingRef = existingAlias.Ref
		if strings.TrimSpace(existingAlias.Ref) == descriptorRef {
			if version != "" && existingAlias.Version != "" && existingAlias.Version != version {
				conflicts = append(conflicts, contributionInstallConflict{
					Kind:          "version_conflict",
					Severity:      "warning",
					Message:       fmt.Sprintf("Alias %q already imports %q at version %q while the contribution metadata reports %q.", selectedAlias, descriptorRef, existingAlias.Version, version),
					ExistingAlias: existingAlias.Alias,
					ExistingRef:   existingAlias.Ref,
					ProposedAlias: selectedAlias,
					ProposedRef:   descriptorRef,
				})
				warnings = append(warnings, fmt.Sprintf("Existing alias %q already imports the same ref with version %q.", selectedAlias, existingAlias.Version))
				proposedImport.Action = "reuse_existing"
				proposedImport.Note = "Target app already imports the same ref under the proposed alias."
			...
			conflicts = append(conflicts, contributionInstallConflict{
				Kind:          "alias_conflict",
				Severity:      "error",
				Message:       fmt.Sprintf("Alias %q already points to %q in the target app.", selectedAlias, existingAlias.Ref),
				ExistingAlias: existingAlias.Alias,
				ExistingRef:   existingAlias.Ref,
				ProposedAlias: selectedAlias,
				ProposedRef:   descriptorRef,
			...
			diagnostics = append(diagnostics, diagnostic{
				Code:     "flogo.contrib.install_plan.alias_conflict",
				Message:  fmt.Sprintf("Alias %q already points to %q.", selectedAlias, existingAlias.Ref),
				Severity: "error",
				Path:     "imports." + selectedAlias,
			})
			proposedImport.Action = "conflict"
			proposedImport.Note = "The proposed alias is already bound to a different ref in the target app."
			installReady = false
			readiness = lowerInstallReadiness(readiness, "low")
		}
	} else if existingRefs := findImportsByRef(app, descriptorRef); len(existingRefs) > 0 {
		existing := existingRefs[0]
		proposedImport.Action = "reuse_existing"
		proposedImport.ExistingAlias = existing.Alias
		proposedImport.ExistingRef = existing.Ref
		proposedImport.Note = "Target app already imports the same ref under a different alias."
		if strings.TrimSpace(request.PreferredAlias) != "" && normalizeAlias(request.PreferredAlias) != normalizeAlias(existing.Alias) {
			warnings = append(warnings, fmt.Sprintf("Preferred alias %q was not selected because the target app already imports %q as %q.", request.PreferredAlias, descriptorRef, existing.Alias))
		}
		conflicts = append(conflicts, contributionInstallConflict{
			Kind:          "ref_already_imported",
			Severity:      "warning",
			Message:       fmt.Sprintf("Target app already imports %q as alias %q.", descriptorRef, existing.Alias),
			ExistingAlias: existing.Alias,
			ExistingRef:   existing.Ref,
			ProposedAlias: selectedAlias,
			ProposedRef:   descriptorRef,
		})
		selectedAlias = existing.Alias
		proposedImport.Alias = existing.Alias
		readiness = lowerInstallReadiness(readiness, "medium")
	}
	...
	if !request.Result.Validation.Ok || !request.Result.Build.Ok || !request.Result.Test.Ok {
		...
		conflicts = append(conflicts, contributionInstallConflict{
			...
			Severity:      "error",
			Message:       "Contribution proof is incomplete or failing; install planning cannot claim high readiness.",
			ProposedAlias: selectedAlias,
			ProposedRef:   descriptorRef,
		})
		diagnostics = append(diagnostics, diagnostic{
			Code:     "flogo.contrib.install_plan.proof_incomplete",
	...
	if request.Package == nil {
		warnings = append(warnings, "No packaged archive was supplied, so install planning is based on scaffold and proof metadata only.")
		readiness = lowerInstallReadiness(readiness, "medium")
	}

	if strings.TrimSpace(bundle.Descriptor.Alias) == "" {
		warnings = append(warnings, "The contribution descriptor does not declare an alias, so the install alias is inferred from the bundle metadata.")
		readiness = lowerInstallReadiness(readiness, "medium")
	}

	if request.ReplaceExisting {
		warnings = append(warnings, "replaceExisting is planning-only in this slice; no imports or refs were updated automatically.")
	}

	surface, surfaceNote := contributionInstallSurface(bundle.Kind)
	reusableRef := contributionInstallRefEntry{
		Surface: surface,
		Value:   "#" + normalizeAlias(selectedAlias),
		Note:    surfaceNote,
	}

	predictedChanges := contributionInstallPredictedChanges{
		ImportsToAdd:    []contributionInstallImportEntry{},
		ImportsToUpdate: []contributionInstallImportEntry{},
		ReusableRefs:    []contributionInstallRefEntry{reusableRef},
		SummaryLines:    []string{},
		NoMutation:      true,
	}

	switch proposedImport.Action {
	case "add":
		predictedChanges.ImportsToAdd = append(predictedChanges.ImportsToAdd, proposedImport)
		predictedChanges.SummaryLines = append(predictedChanges.SummaryLines, fmt.Sprintf("Add import alias %q for ref %q.", proposedImport.Alias, proposedImport.Ref))
	case "reuse_existing":
		if proposedImport.ExistingAlias != "" {
			predictedChanges.SummaryLines = append(predictedChanges.SummaryLines, fmt.Sprintf("Reuse existing import alias %q for ref %q.", proposedImport.ExistingAlias, descriptorRef))
		}
	case "update_existing":
		predictedChanges.ImportsToUpdate = append(predictedChanges.ImportsToUpdate, proposedImport)
		predictedChanges.SummaryLines = append(predictedChanges.SummaryLines, fmt.Sprintf("Review an import update for alias %q to point at %q.", proposedImport.Alias, proposedImport.Ref))
	default:
		predictedChanges.SummaryLines = append(predictedChanges.SummaryLines, fmt.Sprintf("Do not install automatically because alias %q conflicts with an existing import.", proposedImport.Alias))
	}
	predictedChanges.SummaryLines = append(predictedChanges.SummaryLines, fmt.Sprintf("If approved later, the contribution would become available as %q for %s usage.", reusableRef.Value, surface))

	recommendedNextAction := fmt.Sprintf("Review adding import alias %q for ref %q before applying any app mutation.", selectedAlias, descriptorRef)
	if containsInstallConflict(conflicts, "alias_conflict") {
		recommendedNextAction = fmt.Sprintf("Choose a different alias than %q or plan a later replace/update workflow before installation.", selectedAlias)
	} else if containsInstallConflict(conflicts, "ref_already_imported") {
		recommendedNextAction = fmt.Sprintf("Reuse the existing alias %q when planning later app mutations that reference %q.", selectedAlias, descriptorRef)
	} else if containsInstallConflict(conflicts, "version_conflict") {
		recommendedNextAction = fmt.Sprintf("Review version compatibility for alias %q before treating this contribution as install-ready.", selectedAlias)
	} else if containsInstallConflict(conflicts, "insufficient_metadata") {
		recommendedNextAction = "Re-run contribution validation/package proof before attempting an install or update workflow."
	}

	limitations := []string{
		"Planning only; this slice does not mutate flogo.json or install the contribution into the target app.",
		"Trigger, action, and activity usage wiring remains a separate later workflow after import planning is reviewed.",
	}
	if request.Package == nil {
		limitations = append(limitations, "Install readiness is inferred without a packaged archive and may be reduced when package metadata is unavailable.")
	}
	if request.ReplaceExisting {
		limitations = append(limitations, "replaceExisting is advisory only in this slice and does not apply updates.")
	}
...
func validateActionScaffoldRequest(request actionScaffoldRequest) error {
	problems := []string{}
	if strings.TrimSpace(request.ActionName) == "" {
		problems = append(problems, "actionName is required")
	}
	if strings.TrimSpace(request.ModulePath) == "" {
		problems = append(problems, "modulePath is required")
	}
	...
	for _, group := range []struct {
		...
		for _, field := range group.fields {
			...
			case "string", "integer", "number", "boolean", "object", "array", "any":
			default:
				problems = append(problems, fmt.Sprintf("unsupported action scaffold field type %q in %s", field.Type, group.name))
			}
		}
	}
...
func buildScaffoldedActionDescriptor(request actionScaffoldRequest, packageName string) contribDescriptor {
	descriptorName := slugify(valueOrFallback(request.ActionName, packageName))
	return contribDescriptor{
		Ref:      strings.TrimSpace(request.ModulePath),
		Alias:    packageName,
		Type:     "action",
		...
		Outputs:  request.Outputs,
		Examples: []string{
			fmt.Sprintf("Import %q and bind %q as a reusable action in a trigger handler or internal flow adapter.", strings.TrimSpace(request.ModulePath), descriptorName),
		},
		Source: "action_scaffold",
		CompatibilityNotes: []string{
			"Generated by the Phase 4.3 action scaffold foundation.",
			"Review factory wiring, input/output metadata, and runtime action registration before installing the action into an application.",
			"Action scaffolding is narrower than the Activity and Trigger authoring slices and is based on the repo's current core action model.",
		},
	}
}
...
func writeScaffoldedActionFiles(bundleRoot string, request actionScaffoldRequest, descriptor contribDescriptor, packageName string) ([]generatedContribFile, error) {
	if err := os.MkdirAll(bundleRoot, 0o755); err != nil {
		return nil, err
	}
	...
	if err := write("descriptor.json", "descriptor", renderActionDescriptorJSON(request, descriptor)); err != nil {
		return nil, err
	}
	if err := write("go.mod", "module", renderContributionGoMod(strings.TrimSpace(request.ModulePath))); err != nil {
		return nil, err
	}
	if err := write("metadata.go", "metadata", renderActionMetadata(request, packageName)); err != nil {
		return nil, err
	}
	if err := write("action.go", "implementation", renderActionImplementation(request, packageName)); err != nil {
		return nil, err
	}
	if err := write("action_test.go", "test", renderActionTest(request, packageName)); err != nil {
		return nil, err
	}
	if err := write("README.md", "readme", renderActionReadme(request, descriptor, packageName)); err != nil {
		return nil, err
	}
...
func renderActionDescriptorJSON(request actionScaffoldRequest, descriptor contribDescriptor) string {
	payload := map[string]any{
		"name":        descriptor.Name,
		"type":        "flogo:action",
		"version":     descriptor.Version,
...
func renderActionMetadata(request actionScaffoldRequest, packageName string) string {
...
func renderActionImplementation(request actionScaffoldRequest, packageName string) string {
	var builder strings.Builder
	builder.WriteString("package " + packageName + "\n\n")
	builder.WriteString("import (\n")
	builder.WriteString("\t\"context\"\n\n")
	builder.WriteString("\t\"github.com/project-flogo/core/action\"\n")
	builder.WriteString("\t\"github.com/project-flogo/core/data/metadata\"\n")
	builder.WriteString(")\n\n")
	builder.WriteString("func init() {\n")
	builder.WriteString("\t_ = action.Register(&Action{}, &ActionFactory{})\n")
	builder.WriteString("}\n\n")
	builder.WriteString("var actionMd = action.ToMetadata(&Settings{}, &Input{}, &Output{})\n\n")
	builder.WriteString("// ActionFactory creates configured action instances for the generated scaffold.\n")
	builder.WriteString("type ActionFactory struct{}\n\n")
	builder.WriteString("func (f *ActionFactory) Initialize(ctx action.InitContext) error {\n")
	builder.WriteString("\treturn nil\n")
	builder.WriteString("}\n\n")
	builder.WriteString("func (f *ActionFactory) New(config *action.Config) (action.Action, error) {\n")
	builder.WriteString("\tsettings := &Settings{}\n")
	builder.WriteString("\tif err := metadata.MapToStruct(config.Settings, settings, true); err != nil {\n")
	builder.WriteString("\t\treturn nil, err\n\t}\n")
	builder.WriteString("\treturn &Action{settings: settings}, nil\n")
	builder.WriteString("}\n\n")
	builder.WriteString("// Action is a scaffolded Flogo action. Review Run and metadata before production use.\n")
	builder.WriteString("type Action struct {\n")
	builder.WriteString("\tsettings *Settings\n")
	builder.WriteString("}\n\n")
	builder.WriteString("func (a *Action) Metadata() *action.Metadata {\n")
	builder.WriteString("\treturn actionMd\n")
	builder.WriteString("}\n\n")
	builder.WriteString("func (a *Action) IOMetadata() *metadata.IOMetadata {\n")
	builder.WriteString("\treturn actionMd.IOMetadata\n")
	builder.WriteString("}\n\n")
	builder.WriteString("func (a *Action) Run(ctx context.Context, inputValues map[string]interface{}) (map[string]interface{}, error) {\n")
	builder.WriteString("\tinput := &Input{}\n")
	builder.WriteString("\tif err := input.FromMap(inputValues); err != nil {\n")
	builder.WriteString("\t\treturn nil, err\n\t}\n")
	builder.WriteString("\toutput := &Output{}\n")
	for _, field := range request.Outputs {
		matchingInput := false
		for _, inputField := range request.Inputs {
			if strings.EqualFold(inputField.Name, field.Name) {
				matchingInput = true
				builder.WriteString(fmt.Sprintf("\toutput.%s = input.%s\n", exportedIdentifier(field.Name), exportedIdentifier(inputField.Name)))
				break
			}
		}
		if !matchingInput {
	...
	builder.WriteString("\treturn output.ToMap(), nil\n")
	builder.WriteString("}\n")
	return builder.String()
}
...
func renderActionTest(request actionScaffoldRequest, packageName string) string {
	var builder strings.Builder
	builder.WriteString("package " + packageName + "\n\n")
	builder.WriteString("import (\n")
	builder.WriteString("\t\"context\"\n")
	...
	builder.WriteString(")\n\n")
	builder.WriteString("func TestRegister(t *testing.T) {\n")
	builder.WriteString("\tref := support.GetRef(&Action{})\n")
	builder.WriteString("\tif action.GetFactory(ref) == nil {\n")
	builder.WriteString("\t\tt.Fatalf(\"expected action factory %s to be registered\", ref)\n\t}\n")
	builder.WriteString("}\n\n")
	builder.WriteString("func TestFactoryNew(t *testing.T) {\n")
	builder.WriteString("\tfactory := &ActionFactory{}\n")
	builder.WriteString("\tinstance, err := factory.New(&action.Config{\n")
	...
	builder.WriteString("\t})\n")
	builder.WriteString("\tif err != nil {\n")
	builder.WriteString("\t\tt.Fatalf(\"new action: %v\", err)\n\t}\n")
	builder.WriteString("\tif instance == nil {\n")
	builder.WriteString("\t\tt.Fatal(\"expected action instance\")\n\t}\n")
	builder.WriteString("\tif factory.Initialize(nil) != nil {\n")
	builder.WriteString("\t\tt.Fatal(\"expected initialize to be a no-op\")\n\t}\n")
	builder.WriteString("}\n\n")
	builder.WriteString("func TestRun(t *testing.T) {\n")
	builder.WriteString("\tact := &Action{}\n")
	builder.WriteString("\tinputMap := map[string]interface{}{\n")
	for _, field := range request.Inputs {
		builder.WriteString(fmt.Sprintf("\t\t%q: %s,\n", field.Name, goLiteralForField(field)))
	}
	builder.WriteString("\t}\n")
	builder.WriteString("\tresults, err := act.Run(context.Background(), inputMap)\n")
	builder.WriteString("\tif err != nil {\n")
	builder.WriteString("\t\tt.Fatalf(\"run action: %v\", err)\n\t}\n")
	builder.WriteString("\toutput := &Output{}\n")
	builder.WriteString("\tif err := output.FromMap(results); err != nil {\n")
	builder.WriteString("\t\tt.Fatalf(\"decode output: %v\", err)\n\t}\n")
	builder.WriteString("}\n")
	return builder.String()
}
...
func renderActionReadme(request actionScaffoldRequest, descriptor contribDescriptor, packageName string) string {
	var builder strings.Builder
	builder.WriteString("# " + valueOrFallback(strings.TrimSpace(request.Title), request.ActionName) + "\n\n")
	builder.WriteString(strings.TrimSpace(request.Description) + "\n\n")
	builder.WriteString("## Generated bundle\n\n")
	builder.WriteString("- module path: `" + strings.TrimSpace(request.ModulePath) + "`\n")
	builder.WriteString("- package name: `" + packageName + "`\n")
	builder.WriteString("- action ref: `" + descriptor.Ref + "`\n")
	builder.WriteString("- version: `" + descriptor.Version + "`\n\n")
	builder.WriteString("## Files\n\n")
	builder.WriteString("- `descriptor.json`\n")
	builder.WriteString("- `go.mod`\n")
	builder.WriteString("- `metadata.go`\n")
	builder.WriteString("- `action.go`\n")
	builder.WriteString("- `action_test.go`\n\n")
	builder.WriteString("## Review note\n\n")
	builder.WriteString("This is a scaffold foundation bundle. Review action registration, input/output mapping, and `Run` behavior before wiring the action into an app.\n")
