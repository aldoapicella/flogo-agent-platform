...
go run ./go-runtime/flogo-helper contrib install-apply --app examples/hello-rest/flogo.json --request contrib-install-apply-request.json
go run ./go-runtime/flogo-helper contrib update-plan --app examples/hello-rest/flogo.json --request contrib-update-plan-request.json
go run ./go-runtime/flogo-helper contrib update-diff-plan --app examples/hello-rest/flogo.json --request contrib-update-diff-plan-request.json
go run ./go-runtime/flogo-helper contrib update-apply --app examples/hello-rest/flogo.json --request contrib-update-apply-request.json
go run ./go-runtime/flogo-helper contrib uninstall-plan --app examples/hello-rest/flogo.json --request contrib-uninstall-plan-request.json
```

The contribution lifecycle remains narrow and review-oriented. The scaffold commands generate reviewable contribution bundles in a temporary workspace, `validate` and `package` accept one existing scaffold result or persisted `contrib_bundle` artifact, `install-plan` accepts one existing bundle/package source plus one target app, `install-diff-plan` accepts one existing install-plan source plus one target app, `install-apply` accepts one existing exact diff preview source plus one target app, `update-plan` accepts one existing bundle/package source plus one target app where a compatible contribution is already installed, `update-diff-plan` accepts one existing update-plan source plus one target app, `update-apply` accepts one existing exact update diff preview source plus one target app, and `uninstall-plan` accepts one installed-contribution selector plus one target app. All twelve commands reuse isolated `go mod tidy`, `go test ./...`, and `go build ./...` proof where applicable, with packaging additionally emitting one conservative review archive, diff planning additionally materializing the exact canonical `flogo.json` preview without mutation, install apply revalidating that preview before writing the saved canonical mutation to the target app, update planning conservatively detecting installed alias/ref/import state without mutation, update diff planning materializing the exact canonical before/after preview without mutation, update apply revalidating the reviewed diff before writing the saved canonical mutation to the target app, and uninstall planning conservatively surfacing direct usages, orphan risks, and blocked reasons without mutation.
The control-plane now requires the shared Blob/Azurite storage seam to be configured before contribution scaffold, validate, package, install-plan, install-diff-plan, install-apply, update-plan, update-diff-plan, update-apply, or uninstall-plan tasks are synced; it fails loudly rather than silently keeping those authoring artifacts on `memory://` URIs.
