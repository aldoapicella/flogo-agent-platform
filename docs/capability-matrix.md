...
| Contribution factory | Contribution validation/build/test | Validate generated contrib bundles | `runner.scaffoldActivity`, `runner.scaffoldAction`, `runner.scaffoldTrigger`, helper `contrib scaffold-activity`, helper `contrib scaffold-action`, helper `contrib scaffold-trigger` | Shared runner/helper proof shaping plus Blob-backed build/test evidence for scaffolded Activity, Action, and Trigger bundles | V4 | Partial | Implemented only for the generated Activity, Action, and Trigger scaffold slices. General contribution validation/build/test/package/install flows remain later work |
