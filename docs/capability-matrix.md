...
| Platform support | Analysis artifact persistence | Persist app-analysis payloads durably | `GET /catalog`, `GET /descriptors`, `POST /mappings/preview`, `GET /artifacts` | Blob/Azurite-backed JSON payload plus Prisma metadata | V1 | Implemented | Catalog, descriptor, and mapping-preview analysis artifacts now persist payloads to storage |
