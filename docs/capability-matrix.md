...
| Contribution factory | Contribution validation/build/test | Validate generated contrib bundles | `runner.scaffoldActivity`, helper `contrib scaffold-activity` | Runner build/test evidence for scaffolded Activity bundles | V4 | Partial | Implemented only for the generated Activity scaffold slice. General contribution validation/build/test/package/install flows remain later work |
