...
- uninstall/replacement planning and broader automatic install/update into apps remain later work.

## Current Implementation Baseline

The current codebase has a completed Phase 1 foundation, an implemented Phase 2 design surface with the limitations noted above, a partially runtime-backed Phase 3 runtime-evidence surface with a landed Phase 3.2 recorder-backed/narrow-replay foundation on one supported slice, and a narrow Phase 4 Activity/Trigger/Action authoring foundation plus shared validate/package/install-plan/install-diff-plan/install-apply/update-plan/update-diff-plan/update-apply groundwork rather than only placeholders.
...
- uninstall/replacement flows beyond the new Activity/Trigger/Action scaffold plus shared validate/package/install-plan/install-diff-plan/install-apply/update-plan/update-diff-plan/update-apply foundation
...
| --- | --- | --- | --- | --- |
| Shared contracts | Inventory, descriptor, contribution-evidence, governance, composition, mapping-preview, mapping-test, property-plan, flow-contract, trigger-binding, subflow-refactor, advanced control-flow, run-trace, replay, run-comparison, diagnosis, and shared activity/action/trigger-authoring plus install-planning/update-planning/diff-planning/apply contracts | Partial | `packages/contracts/src/index.ts` | Add uninstall-planning contracts later |
...
The next implementation slice after the current baseline should keep contribution authoring reviewable while moving from the now-complete review-gated update/apply chain into uninstall/replacement planning or richer authoring UX on top of the existing scaffold/validate/package/install-plan/update-plan/update-diff-plan/install-diff-plan/install-apply/update-apply lifecycle.

Recommended next items:

1. Add uninstall or replacement planning so the contribution lifecycle can handle safe reviewable removal/change paths distinct from install/update.
