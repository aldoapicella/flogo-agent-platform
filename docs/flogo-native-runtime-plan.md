...
- review-gated update apply and automatic install/update into apps remain later work.

## Current Implementation Baseline

The current codebase has a completed Phase 1 foundation, an implemented Phase 2 design surface with the limitations noted above, a partially runtime-backed Phase 3 runtime-evidence surface with a landed Phase 3.2 recorder-backed/narrow-replay foundation on one supported slice, and a narrow Phase 4 Activity/Trigger/Action authoring foundation plus shared validate/package/install-plan/install-diff-plan/install-apply/update-plan/update-diff-plan groundwork rather than only placeholders.
...
- review-gated contribution update/apply flows beyond the new Activity/Trigger/Action scaffold plus shared validate/package/install-plan/install-diff-plan/install-apply/update-plan/update-diff-plan foundation
...
| Tool layer | Flogo core/mapping tools split plus flow-contract, trigger-binding, subflow-refactor, advanced control-flow dispatch, runtime trace dispatch, replay dispatch, run-comparison dispatch, diagnosis dispatch, and shared contribution scaffold/validate/package/install-plan/update-plan/update-diff-plan/install-diff-plan/install-apply dispatch | Partial | `packages/tools/src/*.ts` | Add review-gated update-apply authoring tool modules later |
| Planner | Analysis-only modes, runtime trace/replay/run-comparison/diagnosis planning-execution routing, Flogo-aware step selection, and narrow activity/action/trigger scaffold plus shared validate/package/install-plan/update-plan/update-diff-plan/install-diff-plan/install-apply authoring routes | Partial | `packages/agent/src/index.ts` | Expand authoring heuristics beyond the current scaffold families later |
| Control-plane APIs | Graph, inventory, catalog, descriptor inspection, contribution evidence inspection, flow contracts, runtime trace, replay, run comparison, trigger binding, subflow extraction/inlining, iterator/retry/doWhile/error-path synthesis, governance, composition comparison, mapping preview, mapping test, property plan, activity/action/trigger scaffold tasks, shared contribution validate/package/install-plan/update-plan/update-diff-plan/install-diff-plan/install-apply tasks, app artifact listing | Partial | `apps/control-plane/src/modules/flogo-apps/*` | Add dedicated contribution-authoring APIs later if needed |
| Persistence | Prisma-backed task/event/artifact state plus hidden app-analysis records, Blob-backed analysis payloads, and Blob-backed contribution authoring bundle/validation/package/install-plan/update-plan/update-diff-plan/install-diff-plan/install-apply payloads | Partial | `apps/control-plane/src/modules/agent/task-store.service.ts`, `apps/control-plane/src/modules/flogo-apps/app-analysis-storage.service.ts` | Extend Blob-backed storage to broader runtime/task artifacts beyond the current analysis and contribution-authoring slices |
| Runner-worker | Flow-contract, runtime trace, replay, run comparison, trigger-binding, subflow extraction/inlining, iterator/retry/doWhile/error-path synthesis, inventory, catalog, descriptor, contribution evidence, governance, composition comparison, mapping preview, mapping test, property-plan, diagnosis, and shared contribution scaffold/validate/package/install-plan/update-plan/update-diff-plan/install-diff-plan/install-apply execution support | Partial | `apps/runner-worker/src/services/*` | Add review-gated update-apply authoring job kinds later |
| Go helper | Flow-contract, runtime trace, replay, run comparison, trigger-binding, subflow extraction/inlining, iterator/retry/doWhile/error-path synthesis, inventory, catalog, descriptor, contribution evidence, governance, composition comparison, mapping preview, mapping test, property planning, and shared Activity/Action/Trigger scaffold/validate/package/install-plan/update-plan/update-diff-plan/install-diff-plan/install-apply execution | Partial | `go-runtime/flogo-helper/main.go` | Expand contribution authoring into review-gated update/apply workflows |
| Web console | Task detail runtime-evidence inspection plus diagnosis-summary rendering for trace, replay, compare, diagnosis, and contribution authoring/install-plan/update-plan/update-diff-plan artifacts | Partial | `apps/web-console` | Add richer compare workflows and later review-gated update/apply previews |
...
The next implementation slice after the current baseline should keep contribution authoring reviewable while moving from exact `update_contrib_diff_plan` preview into review-gated update/apply on top of the existing scaffold/validate/package/install-plan/update-plan/update-diff-plan/install-diff-plan/install-apply lifecycle.

Recommended next items:

1. Add `update_contrib_apply` so an exact update diff preview can become a review-gated, approval-oriented canonical mutation workflow without re-planning the update from scratch.
2. Keep update/apply additive, approval-oriented, and explicit about drift or stale-plan failure.
3. Add later authoring UX and update/apply previews only after the shared contribution lifecycle, install planning, update planning, exact diff preview, and review-gated install/apply remain stable.
