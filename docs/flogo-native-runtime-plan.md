...
- partially implemented in three narrow scaffold slices plus nine shared authoring-generalization slices: custom Activity scaffolding, custom Trigger scaffolding, and a narrower custom Action scaffolding slice now exist as analysis-oriented task modes that generate descriptor metadata plus Go/module/test/readme files, shared `validate_contrib` / `package_contrib` task modes can now re-run proof and package one existing scaffold bundle reviewably, `install_contrib_plan` can now analyze one existing bundle/package against one target app reviewably, `install_contrib_diff_plan` can now materialize the exact canonical `flogo.json` preview for one existing install plan without mutating the target app, `install_contrib_apply` can now consume that exact diff preview review-gated and write the saved canonical mutation to one target app, `update_contrib_plan` can now analyze one existing installed contribution conservatively without mutating the target app, `update_contrib_diff_plan` can now materialize the exact canonical `flogo.json` preview for one existing update plan without mutating the target app, `update_contrib_apply` can now consume that exact reviewed diff preview review-gated and write the saved canonical mutation to one target app, and `uninstall_contrib_plan` can now analyze one installed contribution conservatively to surface safe-removal readiness, orphan risks, and blocked-by evidence without mutating the target app,
- the resulting `contrib_bundle`, `contrib_validation_report`, `contrib_package`, `contrib_install_plan`, `contrib_install_diff_plan`, `contrib_install_apply_result`, `contrib_update_plan`, `contrib_update_diff_plan`, `contrib_update_apply`, `contrib_uninstall_plan`, `flogo_json`, `build_log`, and `test_report` artifacts are now persisted through the control-plane task pipeline and uploaded through the Blob/Azurite storage seam used for app-analysis payloads,
...
- `update_contrib_diff_plan` is now implemented as the narrow Phase 4.9 analysis-only slice that consumes one prior update plan, validates that the target app still matches the planning basis, and emits the exact canonical diff preview without mutating `flogo.json`,
- `update_contrib_apply` is now implemented as the narrow Phase 4.10 review-gated update/apply slice that consumes one prior exact update diff preview, revalidates drift, writes the exact saved canonical mutation to `flogo.json`, and persists both an apply artifact and the resulting canonical app output,
- `uninstall_contrib_plan` is now implemented as the narrow Phase 4.11 analysis-only uninstall-planning slice that inspects installed contribution/import/ref evidence, blocks unsafe removal when live usages or orphan risks remain, and persists a conservative uninstall plan without mutating `flogo.json`,
- uninstall diff/apply, replacement workflows, and broader automatic install/update into apps remain later work.

## Current Implementation Baseline

The current codebase has a completed Phase 1 foundation, an implemented Phase 2 design surface with the limitations noted above, a partially runtime-backed Phase 3 runtime-evidence surface with a landed Phase 3.2 recorder-backed/narrow-replay foundation on one supported slice, and a narrow Phase 4 Activity/Trigger/Action authoring foundation plus shared validate/package/install-plan/install-diff-plan/install-apply/update-plan/update-diff-plan/update-apply/uninstall-plan groundwork rather than only placeholders.
...
- `contrib update-plan`
- `contrib update-diff-plan`
- `contrib update-apply`
- `contrib uninstall-plan`

### Not implemented yet

- Core-native programmatic app composition
- uninstall diff/apply and replacement flows beyond the new Activity/Trigger/Action scaffold plus shared validate/package/install-plan/install-diff-plan/install-apply/update-plan/update-diff-plan/update-apply/uninstall-plan foundation
...
| --- | --- | --- | --- | --- |
| Shared contracts | Inventory, descriptor, contribution-evidence, governance, composition, mapping-preview, mapping-test, property-plan, flow-contract, trigger-binding, subflow-refactor, advanced control-flow, run-trace, replay, run-comparison, diagnosis, and shared activity/action/trigger-authoring plus install-planning/update-planning/diff-planning/apply/uninstall-planning contracts | Partial | `packages/contracts/src/index.ts` | Add uninstall diff/apply or replacement contracts later |
...
| Tool layer | Flogo core/mapping tools split plus flow-contract, trigger-binding, subflow-refactor, advanced control-flow dispatch, runtime trace dispatch, replay dispatch, run-comparison dispatch, diagnosis dispatch, and shared contribution scaffold/validate/package/install-plan/update-plan/update-diff-plan/install-diff-plan/install-apply/update-apply/uninstall-plan dispatch | Partial | `packages/tools/src/*.ts` | Add uninstall diff/apply authoring tool modules later |
| Planner | Analysis-only modes, runtime trace/replay/run-comparison/diagnosis planning-execution routing, Flogo-aware step selection, and narrow activity/action/trigger scaffold plus shared validate/package/install-plan/update-plan/update-diff-plan/install-diff-plan/install-apply/update-apply/uninstall-plan authoring routes | Partial | `packages/agent/src/index.ts` | Expand authoring heuristics beyond the current scaffold families later |
| Control-plane APIs | Graph, inventory, catalog, descriptor inspection, contribution evidence inspection, flow contracts, runtime trace, replay, run comparison, trigger binding, subflow extraction/inlining, iterator/retry/doWhile/error-path synthesis, governance, composition comparison, mapping preview, mapping test, property plan, activity/action/trigger scaffold tasks, shared contribution validate/package/install-plan/update-plan/update-diff-plan/install-diff-plan/install-apply/update-apply/uninstall-plan tasks, app artifact listing | Partial | `apps/control-plane/src/modules/flogo-apps/*` | Add dedicated contribution-authoring APIs later if needed |
| Persistence | Prisma-backed task/event/artifact state plus hidden app-analysis records, Blob-backed analysis payloads, and Blob-backed contribution authoring bundle/validation/package/install-plan/update-plan/update-diff-plan/install-diff-plan/install-apply/update-apply/uninstall-plan payloads | Partial | `apps/control-plane/src/modules/agent/task-store.service.ts`, `apps/control-plane/src/modules/flogo-apps/app-analysis-storage.service.ts` | Extend Blob-backed storage to broader runtime/task artifacts beyond the current analysis and contribution-authoring slices |
| Runner-worker | Flow-contract, runtime trace, replay, run comparison, trigger-binding, subflow extraction/inlining, iterator/retry/doWhile/error-path synthesis, inventory, catalog, descriptor, contribution evidence, governance, composition comparison, mapping preview, mapping test, property-plan, diagnosis, and shared contribution scaffold/validate/package/install-plan/update-plan/update-diff-plan/install-diff-plan/install-apply/update-apply/uninstall-plan execution support | Partial | `apps/runner-worker/src/services/*` | Add uninstall diff/apply authoring job kinds later |
| Go helper | Flow-contract, runtime trace, replay, run comparison, trigger-binding, subflow extraction/inlining, iterator/retry/doWhile/error-path synthesis, inventory, catalog, descriptor, contribution evidence, governance, composition comparison, mapping preview, mapping test, property planning, and shared Activity/Action/Trigger scaffold/validate/package/install-plan/update-plan/update-diff-plan/install-diff-plan/install-apply/update-apply/uninstall-plan execution | Partial | `go-runtime/flogo-helper/main.go` | Expand contribution authoring into uninstall diff/apply or replacement workflows |
| Web console | Task detail runtime-evidence inspection plus diagnosis-summary rendering for trace, replay, compare, diagnosis, and contribution authoring/install-plan/update-plan/update-diff-plan/update-apply/uninstall-plan artifacts | Partial | `apps/web-console` | Add richer compare workflows and later uninstall/replacement previews |
| Eval coverage | Existing create/update/debug/review baseline plus dedicated runtime-evidence and diagnosis-confidence suites for the current real runtime slices and targeted Activity/Action/Trigger scaffold coverage | Partial | `packages/evals` | Add broader UI-facing workflow evals and later uninstall diff/apply authoring cases |
...
The next implementation slice after the current baseline should keep contribution authoring reviewable while moving from the new uninstall-planning slice into exact uninstall diff preview, later review-gated uninstall apply, or richer authoring UX on top of the existing scaffold/validate/package/install-plan/update-plan/update-diff-plan/install-diff-plan/install-apply/update-apply/uninstall-plan lifecycle.

Recommended next items:

1. Add exact uninstall diff preview so the contribution lifecycle can materialize the canonical `flogo.json` before/after preview for one reviewed uninstall plan without mutation.
2. Keep lifecycle expansion additive, approval-oriented where mutation is involved, and explicit about drift or stale-plan failure.
3. Add later authoring UX only after scaffold, proof, install, and update workflows remain stable.
